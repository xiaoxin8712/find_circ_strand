...
parser.add_option("-f","--first_read",dest="first_read",type=int,default=1,help="first read is original RNA? 1=yes,0=no")
...
def find_breakpoints(A,B,read,chrom,margin=options.margin,maxdist=options.maxdist):
    ...
    #print "readlen",L
    #print " "*2+read
    #options.asize--anchor size, default 20
    ...
    pair_start = min(A.aend,B.pos)
    pair_end = max(A.aend,B.pos)
    ...
    for x in range(l+1):
        ...
        if dist <= maxdist:
            ...
            if gt == 'GT' and ag == 'AG':
                ...
                #print x,L
                splice = 0
                hits.append((dist,ov,splice,chrom,start,end,'+',pair_start,pair_end))
            elif gt == 'CT' and ag == 'AC':
                #print "FOUND ONE MINUS STRAND"
                splice = 0
                hits.append((dist,ov,splice,chrom,start,end,'-',pair_start,pair_end))
            elif gt == 'AT' and ag == 'AC':
                splice = 1
                hits.append((dist,ov,splice,chrom,start,end,'+',pair_start,pair_end))
            elif gt == 'GT' and ag == 'AT':
                splice = 1
                hits.append((dist,ov,splice,chrom,start,end,'-',pair_start,pair_end))
            else:
                splice = 2
                # + strand is meaningless
                hits.append((dist,ov,splice,chrom,start,end,'+',pair_start,pair_end))            
    ...
    best = hits[0]
    if (best[0] == 0):
	ret = [h for h in hits if ((h[0] == best[0]) and (h[1] == best[1]))]
	starts = set([r[4] for r in ret])
	ends = set([r[5] for r in ret])
	strands = set([r[6]])
	if (len(strands) == 1) and (max(starts)-min(starts)+1 == len(starts)): 
		return [h for h in hits if ((h[0] == best[0]) and (h[1] == best[1]))]
    else:
	return []
sam = pysam.Samfile(sys.argv[1],'r')
try:
    for A,B,C in grouper(3,sam):
        #print A
        #print B
        N['total'] += 1
        if A.is_unmapped or B.is_unmapped or C.is_unmapped:
            N['unmapped'] += 1
            continue
        if A.tid != B.tid:
            N['other_chrom'] += 1
        ...
        if A.is_reverse != B.is_reverse:
            N['other_strand'] += 1
            continue
        if A.is_reverse == C.is_reverse:
            N['same_pair_strand'] += 1
            continue
        ...
        if numpy.abs(dist) < options.asize:
            ...
            continue
        #strand flag, flag=0, C is read2; flag=1, C is read1 (first read).
        flag = int(C.qname.split('_')[-1])-1
        #options.first_read, 1: first read is original RNA, 0: no
        fr = options.first_read
        # flag == fr: strand C = original RNA, flag != fr: strand A&B = original RNA
        if (flag == fr):
            sense_flag = 0
        else:
            sense_flag = 1

        if (A.qname)and((A.is_reverse and dist > 0) or (not A.is_reverse and dist < 0)):
            # get original read sequence
            read0 = A.qname.split('__')[1]
            chrom = sam.getrname(A.tid)
            
            if A.is_reverse:
                #print "ISREVERSE"
                A,B = B,A
                read1 = rev_comp(read0)
                sense1 = '-'
                bp = find_breakpoints(A,B,read1,chrom)
            else:
                sense1 = '+'
		read1 = read0
                bp = find_breakpoints(A,B,read1,chrom)
            ...
            for h in bp:
                ...
                dist,ov,splice,chrom,start,end,sense2,p_start,p_end = h
		#print splice
		#print sense1
		#print sense2
		    ...
		    read = read0
                    ...
                    sense = rev_sense(sense1)
		    read = rev_comp(read0)
		#print sense
		if splice ==0 and sense == sense2:
		    splice = 2
		elif splice == 1 and sense == sense2:
		    splice = 1
		else:
		    splice = 0
                h = (chrom,start+1,end-1,sense,splice)

                #if A.is_reverse:
                    #read = rev_comp(read)
                Cchrom = sam.getrname(C.tid)
                Cstart = min(C.pos,C.aend)
                Cend = max(C.pos,C.aend)
                if (Cchrom == chrom) and (Cstart > start+1) and (Cend < end-1):
        ...
        if (A.qname) and ((A.is_reverse and dist < 0) or (not A.is_reverse and dist > 0)):
            ...
            if A.is_reverse:
                ...
                sense1 = '+'
                bp = find_breakpoints(A,B,read0,chrom)
            if not bp:
                N['splice_no_bp'] += 1
            else:
                N['spliced_reads'] += 1
		...
		if splice == 0 and sense == sense2:
		    ...
		    splice = 0
                h = (chrom,start,end,sense,splice)
...
def output(ofile,rfile,cand,prefix):
    head="\t".join(['chrom','start','end','name','n_reads','strand','n_uniq','splice','best_qual_A','best_qual_B',
        'spliced_at_begin','spliced_at_end','tissues','tiss_counts','edits','anchor_overlap','breakpoints'])
    ofile.write("#%s\n" % head)
    ...
    for c,hit in cand.items():
        #print c
        chrom,start,end,sense,splice = c
        ...
        name = "%s_%s_%06d" % (options.prefix,prefix,n)
        n += 1
        #sys.stderr.write("%s\t%s\n" % (name,"\t".join(sorted(reads))))
        for r in hit.reads:
            rfile.write("%s\t%s\n" % (name,r))
        
        bed = [chrom,start-1,end,name,n_reads,sense,n_uniq,splice,best_qual,best_other,spliced_at_begin,spliced_at_end,",".join(tissues),",".join(tiss_counts),min_edit,min_anchor_ov,n_hits]
        item="\t".join([str(b) for b in bed])
        ofile.write("%s\n" % item)
...
rfile=open(sys.argv[3],'w')

output(ofile,rfile,circs,"circ")
output(ofile,rfile,splices,"norm")
